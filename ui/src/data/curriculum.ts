...
export async function getClassData(classId: number): Promise<ClassData | undefined> {
  ...
  const res = await fetch(`http://localhost:3001/class/${classId}`);
...
export async function getChapter(classId: number, chapterId: string) {
  const res = await fetch(
    `http://localhost:3001/chapter?classId=${classId}&chapterId=${chapterId}`
  );
...
export async function getTopic(classId: number, topicId: string) {
  const res = await fetch(
    `http://localhost:3001/topic?classId=${classId}&topicId=${topicId}`
  );
  console.log(res.json());
...
export async function getAllQuestions(
  ...
  let url = `http://localhost:3001/questions?classId=${classId}`;
